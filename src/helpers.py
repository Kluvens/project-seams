...
def create_tagging_notification(sender_u_id, uids, msg_str, channel_id, dm_id):
    if uids:
        ...
        if channel_id == -1:
            destination = get_dm_name(dm_id)
        else:
            destination = get_channel_name(channel_id)

        # Creating notifications for each user in uids list
        for u_id in uids:
            user_idx = get_user_idx(users, u_id)
            
            tag_msg_str = f"{handle} tagged you in {destination}: {msg_substr}"
...
def get_dm_name(dm_id):
    dms = data_store.get()['dms']
    dm_name = [dm['name'] for dm in dms if dm['dm_id'] == dm_id][0]
    return dm_name
