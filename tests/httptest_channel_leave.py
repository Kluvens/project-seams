...
# Authorized user is starter of Active standup in channel
def test_user_is_standup_starter(create_route,leave_route,dummy_data):
    reset_call()

    # Generate user
    user = dummy_data.register_users(num_of_users=1)[0]
    ...
    # Start standup in channel
    requests.post(start_standup, json = {
        'token': user_token,
        'channel_id': channel_id,
        'length': '3600'
    })
