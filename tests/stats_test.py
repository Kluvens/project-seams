...
def test_user_stats_working(setup):
    ...
    assert len(stats_result['user_stats']["channels_joined"]) == 2
    assert len(stats_result['user_stats']["dms_joined"]) == 2
    assert len(stats_result['user_stats']["messages_sent"]) == 1
    ...
    assert len(stats_result['user_stats']["channels_joined"]) == 2
    assert len(stats_result['user_stats']["dms_joined"]) == 2
    assert len(stats_result['user_stats']["messages_sent"]) == 3
    ...
    assert len(stats_result['user_stats']["channels_joined"]) == 2
    assert len(stats_result['user_stats']["dms_joined"]) == 2
    assert len(stats_result['user_stats']["messages_sent"]) == 2 + 10 + 10 + 1
...
def test_usersstats_working(setup):
    ...
    assert len(stats_result['workspace_stats']["channels_exist"]) == 2
    assert len(stats_result['workspace_stats']["dms_exist"]) == 2
    assert len(stats_result['workspace_stats']["messages_exist"]) == 1
